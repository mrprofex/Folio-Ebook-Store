...
var Database = class {
  ...
  async loadEbookMapByIds(ids) {
    const idArray = Array.from(ids);
    if (idArray.length === 0) return /* @__PURE__ */ new Map();
  ...
  async getAllEbooks(options = {}) {
    ...
    const ebookMap = /* @__PURE__ */ new Map();
  ...
  async enrichEbook(book, coupons, ebookMap) {
    ...
    if (!map) {
      const ebookIds = /* @__PURE__ */ new Set([book.id]);
  ...
  async enrichCouponsBatch(coupons) {
    ...
    const purchaseStats = /* @__PURE__ */ new Map();
    for (const r of purchasesRes.rows) {
  ...
  async getDashboardStats() {
    ...
    const revenueByMonth = months.map((m, monthIndex) => {
      ...
      const targetYear = (/* @__PURE__ */ new Date()).getFullYear() - (currentMonth < 5 && monthIndex >= currentMonth ? 1 : 0);
      return {
        month: m,
        revenue: 0,
        sales: 0
...
var UPLOAD_PRESET = process.env.CLOUDINARY_UPLOAD_PRESET || "folio_uploads";
...
async function uploadToCloudinaryUnsigned(buffer, options) {
  ...
  params.append("timestamp", String(Math.floor(Date.now() / 1e3)));
  const parts = [];
        ...
        res.on("end", () => {
          ...
          try {
            ...
            if (res.statusCode !== 200 || result.error) {
              const error = new Error(result.error?.message || `HTTP ${res.statusCode}`);
